...
This document details the NixOS configuration for the new `claw` VPS, following established patterns from `noir`, `zinc`, and `pie` in this repository.

## Host Configuration

### `hosts/claw/default.nix`
...
```nix
{
  ...
  networking = {
    ...
    firewall = {
      enable = true;
      allowedTCPPorts = [80 443]; # ACME + nginx
      # Gateway only accessible over Tailscale
      interfaces.tailscale0.allowedTCPPorts = [18789];
    };
  };
...
### `hosts/claw/hardware-configuration.nix`
...
{config, lib, modulesPath, ...}: {
  ...
  # Disko-managed disk layout for impermanence
  # Root on tmpfs, /nix and /nix/persist on NVMe
  fileSystems = {
    "/" = {
      device = "none";
      fsType = "tmpfs";
    ...
    "/nix" = {
      device = "/dev/sda1"; # Adjust to actual device
      fsType = "ext4";
    };
    "/boot" = {
      device = "/dev/sda15"; # EFI partition
      fsType = "vfat";
    };
  };
...
**Note:** The actual hardware configuration will differ based on Hetzner's disk layout. Use `nixos-generate-config` or disko to produce the correct file during installation.
...
nixosEval-claw = let
  sys = inputs.nixpkgs.lib.nixosSystem {
    ...
    modules = [./hosts/claw/default.nix];
  };
...
in
  pkgs.writeText "nixos-eval-claw.json" summary;
```
...
keys:
  ...
  - &claw age1XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
creation_rules:
...
```bash
ssh root@<claw-ip> "cat /etc/ssh/ssh_host_ed25519_key.pub" | ssh-to-age
```
...
You are **lildoofy_bot**, a personal AI assistant for Brandon running on a NixOS VPS (claw).
...
18. [ ] Set up backups for `/var/lib/clawdbot` and `/var/lib/ollama`
