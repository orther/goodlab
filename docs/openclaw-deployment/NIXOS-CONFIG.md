...
```nix
{
  ...
  # --- SOPS Secrets ---
  # Keep claw isolated from shared repo secrets.
  sops.defaultSopsFile = ./../../secrets/claw-secrets.yaml;
  ...
  services.tailscale = {
    ...
    openFirewall = false;
    # Client mode only: do not advertise homelab routes from claw.
...
Add the new machine's age key and scope it to a dedicated secrets file. Rule order matters: put the `claw` rule before the shared catch-all rule.
...
creation_rules:
  - path_regex: secrets/claw-secrets\.yaml$
    key_groups:
      - age:
          - *stud
          - *claw
...
```bash
ssh orther@<claw-ip> "sudo cat /etc/ssh/ssh_host_ed25519_key.pub" | ssh-to-age
```
...
# Ensure SSH for <hetzner-ip> uses ~/.ssh/claw_admin_ed25519
...
5. [ ] Create `secrets/claw-secrets.yaml` and run `just sopsupdate`
